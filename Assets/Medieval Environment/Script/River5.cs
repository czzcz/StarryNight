...
public class River5 : MonoBehaviour {
	public float speed;
	public float rota;
	float abs;
	...
	void Update () {
		...
		if (transform.localEulerAngles.z < rota && this.GetComponentInChildren <ParticleSystem> ().emissionRate != 0)
			this.GetComponentInChildren <ParticleSystem> ().emissionRate = 0;
		if (transform.localEulerAngles.z > 225 && transform.localEulerAngles.z < 226 && this.GetComponentInChildren <ParticleSystem> ().emissionRate == 0)
